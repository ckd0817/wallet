...
public class WalletDefaultsTest {
    ...
    public void defaultStoreIncludesScreenshotBookkeepingState() {
        ...
        assertTrue(store.has("assetPerformanceHistory"));
        ...
        assertEquals("eastmoney", appSettings.optString("fundQuoteSource"));
    ...
    public void ensureDefaultsUpgradesPreviousDefaultCapturePrompt() throws Exception {
        ...
        assertTrue(currentDefaultPrompt.contains("先用后付"));
        assertTrue(currentDefaultPrompt.contains("将该金额作为 amount 并记为 expense"));
        assertFalse(currentDefaultPrompt.contains("截图不足以确认是一笔有效入账记录"));
        assertFalse(currentDefaultPrompt.contains("\"summary\":\"...\""));
    }

    @Test
    public void ensureDefaultsUpgradesPickupCodeCapturePrompt() throws Exception {
        JSONObject candidate = new JSONObject();
        JSONObject llmConfig = new JSONObject();
        llmConfig.put("capturePrompt", PICKUP_CODE_CAPTURE_PROMPT);
        candidate.put("llmConfig", llmConfig);

        JSONObject merged = WalletDefaults.ensureDefaults(candidate);
        String currentDefaultPrompt = WalletDefaults.defaultLlmConfig().optString("capturePrompt");

        assertEquals(currentDefaultPrompt, merged.optJSONObject("llmConfig").optString("capturePrompt"));
    }
    ...
    public void ensureDefaultsKeepsExpenseAverageMonthsInRange() throws Exception {
        ...
        appSettings.put("fundQuoteSource", "unavailable");
        candidate.put("appSettings", appSettings);

        JSONObject merged = WalletDefaults.ensureDefaults(candidate);

        assertEquals(12, merged.optJSONObject("appSettings").optInt("expenseAverageMonths"));
        assertEquals("eastmoney", merged.optJSONObject("appSettings").optString("fundQuoteSource"));
    }
