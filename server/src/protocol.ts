...
const recordSchemas: Record<string, z.ZodType> = {
  ...
  assetHoldings: z.object({id,assetType: z.enum(['stock','fund']),code: id,market: z.enum(['sh','sz','fund','us']),
    currency:z.enum(['CNY','USD']).optional(),name: text,shares: positive,costAmount: positive,createdAt: date,updatedAt: date}).passthrough(),
  ...
  assetTradeRecords: z.object({id,holdingId:id,assetType:z.enum(['stock','fund']),code:id,name:text,currency:z.enum(['CNY','USD']).optional(),
